...
class ConsultationViewSet(ModelViewSet):
    ...
    def import_finalised_themes(self, request, pk=None) -> Response:
        ...
        for target_q in target_questions.order_by("number"):
            ...
            existing = SelectedTheme.objects.filter(question=target_q)
            if existing.exists():
            ...
            source_themes = SelectedTheme.objects.filter(question=source_q)

            if not source_themes.exists():
                question_info["status"] = "no_themes_in_source"
                questions.append(question_info)
                continue

            question_info["status"] = "will_import"
            question_info["source_themes"] = list(source_themes.values_list("name", flat=True))
            questions.append(question_info)
        ...
        for q_info in questions:
            ...
            target_q = target_questions.get(text=q_info["question_text"])
            source_q = source_questions.get(text=q_info["question_text"])
            themes = SelectedTheme.objects.filter(question=source_q)
