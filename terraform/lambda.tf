...
module "consultation_import_lambda" {
  ...
  handler                      = "consultation_import_handler.lambda_handler"
  runtime                      = "python3.12"
...
module "candidate_themes_import_lambda" {
  source = "git::https://github.com/i-dot-ai/i-dot-ai-core-terraform-modules.git//modules/infrastructure/lambda?ref=v2.0.1-lambda"

  file_path                     = "${path.root}/../lambda/candidate_themes_import.zip"
  handler                      = "candidate_themes_import_handler.lambda_handler"
  runtime                      = "python3.12"
  function_name                = "${local.name}-candidate-themes-import-lambda"
  package_type                 = "Zip"
  memory_size                  = 1024
  source_code_hash             = data.archive_file.candidate_themes_import_archive.output_base64sha256
  account_id                   = data.aws_caller_identity.current.account_id
  lambda_additional_policy_arns = {for idx, arn in [aws_iam_policy.lambda_exec_custom_policy.arn] : idx => arn}
  aws_security_group_ids = [aws_security_group.lambda_sg.id]
  subnet_ids              = data.terraform_remote_state.vpc.outputs.private_subnets
