locals {
  ...
  ecs_env_vars_raw = merge(local.base_env_vars, {
    ...
    "MAPPING_SQS_QUEUE_URL"              = aws_sqs_queue.batch_job_queue.id
    "MAPPING_BATCH_JOB_NAME"             = "${local.name}-mapping-job"
    "MAPPING_BATCH_JOB_QUEUE"            = module.batch_job_mapping.mapping.job_queue_name 
    "MAPPING_BATCH_JOB_DEFINITION"       = module.batch_job_mapping.mapping.job_definition_name  
  })
