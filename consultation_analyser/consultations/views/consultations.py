...
from consultation_analyser.consultations.models import QuestionPart, Question, Consultation, ThemeMapping, Theme
...
def show(request: HttpRequest, consultation_slug: str) -> HttpResponse:
    ...
    for question in questions:
        ...
        if free_text_question_part:
            ...
            theme_mappings_qs = ThemeMapping.get_latest_theme_mappings_for_question_part(free_text_question_part)
            top_counts_by_theme_qs = (theme_mappings_qs.values('theme').annotate(theme_count=Count('theme')).order_by('-theme_count'))[:number_of_themes]
