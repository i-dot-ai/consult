...
def _create_validation_error_response(
    error_type: str, user_message: str, technical_message: str, file_path: str | None = None
) -> dict[str, str]:
    """Create standardized error dictionary structure."""
    response = {"type": error_type, "message": user_message, "technical": technical_message}
    if file_path:
        ...
        if error_message.startswith(pattern):
            ...
            if file_path:
                ...
                return _create_validation_error_response(
                    error_type, message, error_message, file_path
                )
...
def _format_access_errors(error_message: str) -> dict[str, str] | None:
    ...
    if (
        "An error occurred (404)" in error_message
        and "HeadObject operation: Not Found" in error_message
    ):
        file_path = _extract_file_path_from_error_message(error_message)
        if file_path:
            file_name = file_path.split("/")[-1]
            ...
            return _create_validation_error_response(
                "file_not_found", message, error_message, file_path
            )
        ...
        if len(parts) > 1:
            ...
            if file_path:
                ...
                return _create_validation_error_response(
                    "file_error", message, error_message, file_path
                )
