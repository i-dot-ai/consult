...
def _resolve_model(
    ...
    - ``base_url``/``api_key`` given -> gateway mode: build an
      ``OpenAIChatModel`` over an ``OpenAIProvider`` pointed at the gateway.
      Here ``model`` is the gateway's model name (no ``provider:`` prefix); the
      proxy does the routing.
