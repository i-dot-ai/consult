...
def lambda_handler(event, context):
    """
    Lambda function triggered by EventBridge when sign-off Batch job completes.
    Enqueues candidate themes import job to Django RQ.
    """
    ...
    timestamp = parameters.get("mappingDate", "unknown")
    ...
    if not consultation_code or not timestamp:
        error_msg = f"Missing required parameters: consultation_code={consultation_code}, timestamp={timestamp}"
    ...
    try:
        ...
        # Enqueue the candidate themes import job
        logger.info("Enqueueing candidate themes import RQ job...")
        job = queue.enqueue(
            "consultation_analyser.support_console.views.consultations.import_candidate_themes_job",
        ...
        try:
            slack_message = create_slack_success_message(consultation_code, timestamp, job.id)
        ...
        success_msg = f"Successfully queued candidate themes import job {job.id} for consultation: {consultation_code}"
        ...
        error_msg = f"Failed to trigger candidate themes import: {str(e)}"
        logger.error(f"ERROR: {error_msg}")
        logger.error(f"Exception type: {type(e).__name__}")

        # Send error notification to Slack
        try:
            slack_error_message = create_slack_error_message(error_msg, consultation_code)
...
def create_slack_success_message(consultation_code, timestamp, job_id):
    """
    Create Slack message payload for successful job
    """
    return {
        "text": "🎉 Candidate Themes Import Job Queued Successfully",
        "blocks": [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "🎉 Candidate Themes Import Queued"},
            },
            {
                "type": "section",
                "fields": [
...
def create_slack_error_message(error_msg, consultation_code):
    """
    Create Slack message payload for failed job
    """
    return {
        "text": "❌ Candidate Themes Import Job Failed",
        "blocks": [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "❌ Candidate Themes Import Failed"},
            },
            {
                "type": "section",
                "fields": [
