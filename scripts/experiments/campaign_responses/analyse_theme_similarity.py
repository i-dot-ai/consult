...
For each question per run:
  ...
  3. For each test theme, computes a normalised distance to each GT cluster and
     finds the nearest GT theme in each group and compares those two distances.
     Themes where the two nearest distances are within 10% of each other are classified
     as 'both' (overlapping themes present in both groups).
...
def format_dataset_label(name: str, strip: str) -> str:
    """Remove `strip` followed by an optional `_<digits>` suffix from `name` and uppercase the remainder."""
    return re.sub(re.escape(strip) + r"_\d+_?", "", name).upper()
...
def nearest_group(
    ...
    gt1_embeddings: list[np.ndarray],
    gt2_embeddings: list[np.ndarray],
    both_threshold: float = NORM_BOTH_THRESHOLD,
) -> tuple[str, float, float]:
    """Assign to whichever GT group contains the nearest single theme. If the closest theme in
    each group are within `both_threshold` ratio of each other, classify as 'both'.
    Returns (group, gt1_nearest_dist, gt2_nearest_dist)."""
    d1 = min(float(np.linalg.norm(embedding - e)) for e in gt1_embeddings) if gt1_embeddings else float("inf")
    d2 = min(float(np.linalg.norm(embedding - e)) for e in gt2_embeddings) if gt2_embeddings else float("inf")
    if max(d1, d2) > 0 and min(d1, d2) / max(d1, d2) > both_threshold:
        return "both", d1, d2
    return ("1" if d1 <= d2 else "2"), d1, d2
...
def analyse_dataset(
    ...
    Returns {question: (norm_gt1, norm_gt2, norm_both, near_gt1, near_gt2, near_both)}."""
    ...
    with log_path.open("w") as log:
        ...
        for question in questions:
            ...
            k1_label = f"Near({gt1_name[:8]})"
            k2_label = f"Near({gt2_name[:8]})"
            header = f"  Theme | {g1_label} | {g2_label} | Norm Grp | {k1_label} | {k2_label} | Near Grp"
            out(f"\n{header}")
            out("  " + "-" * (len(header) - 2))
            ...
            for label, theme, emb in zip(labels, themes, theme_embeddings):
                ...
                near_winner, kd1, kd2 = nearest_group(emb, gt1_embeddings, gt2_embeddings, norm_both_threshold)
                out(f"  {label:<5} | {d1:.3f}      | {d2:.3f}      | {norm_group:^8} | {kd1:.3f}       | {kd2:.3f}       | {near_winner:^8}")
                if norm_group == "1": norm_gt1 += 1
                elif norm_group == "2": norm_gt2 += 1
                else: norm_both += 1
                if near_winner == "1": knn_gt1 += 1
                elif near_winner == "2": knn_gt2 += 1
                else: knn_both += 1
...
def print_summary(
    ...
    for question, datasets in sorted(summary.items()):
        ...
        print_group_table(
            datasets, name_w, cw,
            headers=("Norm→" + g1, "Norm→both", "Norm→" + g2, "Near→" + g1, "Near→both", "Near→" + g2),
        ...
        print_group_table(
            ...
            headers=(
                ...
                f"Near→{g1}(+both)", f"Near→{g2}(+both)", "Near overlap",
            ),
            fields=(
...
def plot_summary(
    ...
    """For each question, chart the inclusive nearest-theme counts (Near→GT1(+both), Near→GT2(+both),
    Near overlap) per dataset against horizontal reference lines for each GT's consensus size."""
    output_dir.mkdir(parents=True, exist_ok=True)
    gt1_label = format_dataset_label(gt1_name, label_strip)
    gt2_label = format_dataset_label(gt2_name, label_strip)
    ...
    for question, datasets in sorted(summary.items()):
        ...
        g1_means, g1_stds = series("knn_g1_inc_both")  # field names unchanged; now nearest-theme counts
...
def analyse(
    ...
    try:
        print("Loading consensus ground truth embeddings...", file=sys.stderr)
        for question in questions:
        ...
        for dataset_dir in dataset_dirs:
            ...
            for run_num, themes_by_question in sorted(runs.items()):
                ...
                results = analyse_dataset(
                    ...
                    client, cache, log_path, norm_both_threshold=norm_both_threshold,
                )
    ...
    plot_summary(
        summary, gt1_name, gt2_name, gt1_counts_by_question, gt2_counts_by_question,
        search_dir / "charts", dataset_order, label_strip,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label-strip", required=True, help="Substring to remove from directory names when generating chart labels (e.g. 'dwp_7_')")
    parser.add_argument(
    ...
    analyse(
        ...
        norm_both_threshold=args.norm_both_threshold,
