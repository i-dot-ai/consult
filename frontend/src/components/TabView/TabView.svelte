<script lang="ts">
  ...
  export let handleChange = (_newVal: string) => {};
  ...
  function incrementTab() {
    const currTabIndex: number = tabs.findIndex(
      (tab) => tab.id === $writableValue,
    );
    const nextTabIndex: number =
      currTabIndex + 1 >= tabs.length ? 0 : currTabIndex + 1;
...
<div use:melt={$root} class={clsx(["flex", "flex-col", "overflow-hidden"])}>
  <div
    class={clsx([
      "flex",
