<script lang="ts">
    ...
    const toOption = (option: SearchableSelectOption): ComboboxOptionProps<SearchableSelectOption> => ({
        value: option,
        label: option.label,
    ...
    $: filteredOptions = $touchedInput
        ? options.filter(option => {
            return (
                ...
                (option.description && option.description.toLowerCase().includes($inputValue.toLowerCase().trim()))
            );
        })
        : options;
</script>
