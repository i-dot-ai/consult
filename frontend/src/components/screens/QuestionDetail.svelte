<script lang="ts">
  ...
  async function loadData() {
    ...
    console.log(`[QuestionDetail] Starting data load for question ${questionId} in consultation ${consultationId}, page ${currPage}`);
    ...
    if (currPage === 1) {
      ...
      $themeAggrStore.fetch(
        ...
        console.log(`[QuestionDetail] Theme aggregations loaded in ${(performance.now() - themeAggrStart).toFixed(0)}ms`);
      });
      ...
      $themeInfoStore.fetch(
        ...
        console.log(`[QuestionDetail] Theme information loaded in ${(performance.now() - themeInfoStart).toFixed(0)}ms`);
      });
      ...
      $demoOptionsStore.fetch(
        ...
        console.log(`[QuestionDetail] Demographic options loaded in ${(performance.now() - demoOptionsStart).toFixed(0)}ms`);
      });
      ...
      $demoAggrStore.fetch(
        ...
        console.log(`[QuestionDetail] Demographic aggregations loaded in ${(performance.now() - demoAggrStart).toFixed(0)}ms`);
      });
      ...
      $questionStore.fetch(
        ...
        console.log(`[QuestionDetail] Question detail loaded in ${(performance.now() - questionStart).toFixed(0)}ms`);
      });
    }
    ...
    console.log(`[QuestionDetail] Answers (page ${currPage}) loaded in ${(performance.now() - answersStart).toFixed(0)}ms - ${$answersStore.data?.all_respondents?.length || 0} responses`);
    ...
    console.log(`[QuestionDetail] Total data load completed in ${totalDuration.toFixed(0)}ms for question ${questionId}`);
  }
