<script lang="ts">
  ...
  onMount(() => {
    ...
    console.log(
      `[ConsultationDetail] Starting data fetch for consultation ${consultationId}`,
    );

    const questionsPromise = $questionsStore
      .fetch(getApiQuestionsUrl(consultationId))
      .then(() => {
    ...
    const demoPromise = $demoOptionsStore
      .fetch(`/api/consultations/${consultationId}/demographic-options/`)
      .then(() => {
