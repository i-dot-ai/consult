...
<form class={clsx(["flex", "flex-col", "gap-4"])}
  ...
  {/if}
  <TextInput autocomplete="false" id="timestamp" name="timestamp" label="Mapping timestamp folder (only needed to populate the dashboard)" placeholder="e.g. 2024-01-15" value={timestamp} setValue={setTimestamp} />
  <Button
    type="submit"
    variant="primary"
