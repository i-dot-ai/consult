...
const unselectedThemes = allThemes.filter((theme) =>
...
describe("ThemeSelectionForm", () => {
  ...
  it.each(selectedThemes)(
    "should initially select all selected themes",
    async (theme) => {
      mockRoute(showNextMock);
      mockRoute(submitMock);
  ...
  it.each(unselectedThemes)(
    "should initially not select any unselected themes",
    async (theme) => {
      mockRoute(showNextMock);
      mockRoute(submitMock);
  ...
  it("should send request with themes selected when submitted", async () => {
    ...
    const submitButton = screen.getByRole("button", {
      name: "Save and continue to next response",
    });
    await user.click(submitButton);

    await waitFor(() => {
      expect(submitMockCallback).toHaveBeenCalledWith(
  ...
  it("should send request to showNext when skipped", async () => {
    ...
    const user = userEvent.setup();
    const submitButton = screen.getByRole("button", {
      name: "Skip this response",
    });
    await user.click(submitButton);

    await waitFor(() => {
      expect(showNextMockCallback).toHaveBeenCalled();
    });
  });

  it.each(selectedThemes)(
    "should unselect theme when clicked",
    async (theme) => {
      mockRoute(showNextMock);
      mockRoute(submitMock);
