...
describe("AddUserToConsultationForm", () => {
  const mockUsers: User[] = [
    { id: 1, email: "user1@example.com", is_staff: false, has_dashboard_access: false },
    { id: 2, email: "user2@example.com", is_staff: true, has_dashboard_access: true },
  ];
  ...
  it("should submit selected users successfully", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ message: "Successfully added 1 users to consultation" }),
    });
