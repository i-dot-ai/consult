<script lang="ts">
    ...
    let totalResponses = $derived($consultationData?.questions?.reduce(
        (acc, question) => acc + (question?.total_responses || 0),
        0,
    ));

    let demoCategories = $derived([...new Set(($demoOptionsData || []).map(opt => opt.name))]);
...
<section>
    <Panel>
        ...
        <div class="grid grid-cols-12 gap-4 mb-4">
            {#each $consultationData?.questions.filter(
