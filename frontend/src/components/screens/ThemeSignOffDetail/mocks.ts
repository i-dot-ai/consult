...
function selectTheme(themeId: string) {
  ...
  candidateThemes = updateSelectedThemeId(
    candidateThemes,
    themeId,
    newSelectedThemeId,
  );
...
function updateSelectedThemeId(
  themes: Theme[],
  themeId: string,
  newSelectedThemeId: string | null,
) {
  return themes.map((theme): Theme => {
    ...
    if (updatedTheme.children && Array.isArray(updatedTheme.children)) {
      return {
        ...updatedTheme,
        children: updateSelectedThemeId(
          updatedTheme.children,
          themeId,
          newSelectedThemeId,
        ),
      };
...
export const selectedThemesEditMock = {
  regexp:
    "*host" +
    getApiGetSelectedThemeUrl(CONSULTATION_ID, QUESTION_ID, ":themeId"),
  method: "PATCH",
  body: ({ body, params }: { body: string; params: { themeId: string } }) => {
...
export const selectedThemesDeleteMock = {
  regexp:
    ...
    getApiGetSelectedThemeUrl(CONSULTATION_ID, QUESTION_ID, ":themeId"),
  method: "DELETE",
  body: ({ _, params }: { _: string; params: { themeId: string } }) => {
    const themeId = params.themeId;

    selectedThemes = selectedThemes.filter((theme) => theme.id !== themeId);

    const flatCandidateThemes = flatten(candidateThemes);
    const candidateThemeId = flatCandidateThemes.find(
