...
describe("CreateConsultationForm", () => {
  ...
  it("should render consultation",
    async () => {
      ...
      await user.type(screen.getByLabelText("Consultation name"), CONSULTATION_NAME);
      ...
      await waitFor(() => {
        expect(screen.getByText(`${CONSULTATION_NAME} is being set up`, { exact: false })).toBeInTheDocument();
      });
    },
  );

  it("should render error text if fetch fails",
    async () => {
  ...
  it("should render status error text if fetch fails",
    async () => {
      const ERROR_MESSAGE = "Server Error";
      const FOLDER_NAME = "test-folder";
      const CONSULTATION_NAME = "Test Consultation";

      mockRoute({...defaultMock, status: 500, body: { message: ERROR_MESSAGE }});
