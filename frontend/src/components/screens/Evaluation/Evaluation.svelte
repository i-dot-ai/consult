...
<div class={isLoading ? "opacity-60 transition-opacity" : ""}>
  {#if availableUsers.length > 0}
    <div class="mb-6 flex items-start justify-end gap-2">
      <div class="text-right">
        <select
          class="rounded border border-neutral-300 px-3 py-1.5 text-sm text-neutral-700"
          onchange={(e) => {
        ...
        {#if selectedUserIds.length > 0}
          <p class="mt-1 max-w-xs text-xs text-neutral-500">
            Showing only responses read by this user. F1 compares original AI
            assignments against current themes, which may include edits by other
            users.
          </p>
        {/if}
      </div>
