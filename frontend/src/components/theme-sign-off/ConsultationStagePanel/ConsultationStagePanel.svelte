<script lang="ts">
  ...
  // Map old stage names to new ones so both work
  const STAGE_ALIASES: Record<string, string> = {
    finalising_themes: "theme_sign_off",
    theme_mapping: "assigning_themes",
  };

  function resolveStageKey(stage: ConsultationStage): string {
  ...
  let stageKey = $derived(resolveStageKey(consultation.stage));
  let currentStageConfig = $derived(
    STAGE_CONFIGS[stageKey] ?? STAGE_CONFIGS.theme_sign_off,
  );

  // Determine which content variant to show
  let isFinalisingThemes = $derived(stageKey === "theme_sign_off");
  let isAssigningThemes = $derived(stageKey === "assigning_themes");
  let isAnalysis = $derived(stageKey === "analysis");

  // Title adapts to sub-state during finalising
  let title = $derived(
    isFinalisingThemes && allQuestionsFinalised
