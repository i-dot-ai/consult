...
let removeTheme = $state((themeId) => alert(`Remove theme event triggered with: ${themeId}`));
let updateTheme = $state((...args) => alert(`Update theme event triggered with: ${args.join(", ")}`));
