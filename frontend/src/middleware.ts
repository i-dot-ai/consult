...
async function proxyToDjango(context: APIContext, backendUrl: string) {
  ...
  try {
    ...
    console.log(`[CSRF Debug] Path: ${url.pathname}, isAdmin: ${isAdminRequest}, Method: ${context.request.method}, HasCSRFCookie: ${!!csrfCookie}`);
    
    if (csrfCookie && !isAdminRequest) {
      console.log(`[CSRF Debug] Setting X-CSRFToken header for non-admin request`);
