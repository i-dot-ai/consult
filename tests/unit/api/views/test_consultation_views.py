...
class TestGetConsultationFoldersEndpoint:
    def test_get_folders_setup_stage_with_no_consultations(self, client, admin_user_token):
        ...
        with patch(
            ...
            mock_get_folders.return_value = ["healthcare-2024", "transport-2024", "education-2024"]
            ...
            assert response.status_code == 200
            assert response.json() == ["education-2024", "healthcare-2024", "transport-2024"]
    ...
    def test_get_folders_setup_stage_excludes_existing_consultations(
        ...
        # Create consultation with code matching one S3 folder
        ConsultationFactory(code="healthcare-2024", title="Healthcare Consultation")
        ...
        with patch(
            ...
            assert response.status_code == 200
            # Should only return folders without consultations
            assert response.json() == ["education-2024", "transport-2024"]
    ...
    def test_get_folders_find_themes_stage_returns_consultations(self, client, admin_user_token):
        ...
        c1 = ConsultationFactory(code="healthcare-2024", title="Healthcare Consultation")
        c2 = ConsultationFactory(code="transport-2024", title="Transport Consultation")
        ...
        with patch(
            ...
            # Verify sorted by title then code
            assert result[0]["code"] == "healthcare-2024"
            assert result[0]["title"] == "Healthcare Consultation"
            assert result[0]["id"] == str(c1.id)

            assert result[1]["code"] == "transport-2024"
    ...
    def test_get_folders_assign_themes_stage_returns_consultations(self, client, admin_user_token):
        ...
        with patch(
            ...
            mock_get_folders.return_value = ["healthcare-2024", "education-2024"]
    ...
    def test_get_folders_handles_one_to_many_relationship(self, client, admin_user_token):
        ...
        # Create two consultations with the same code (one-to-many relationship)
        c1 = ConsultationFactory(code="healthcare-2024", title="Healthcare Consultation V1")
        c2 = ConsultationFactory(code="healthcare-2024", title="Healthcare Consultation V2")
        ...
        with patch(
            ...
            mock_get_folders.return_value = ["healthcare-2024"]
            ...
            # Both consultations should be returned
            codes_and_ids = [(item["code"], item["id"]) for item in result]
            assert ("healthcare-2024", str(c1.id)) in codes_and_ids
            assert ("healthcare-2024", str(c2.id)) in codes_and_ids

    def test_get_folders_returns_empty_when_no_s3_folders(self, client, admin_user_token):
        """Test endpoint returns empty list when no S3 folders exist"""
        url = reverse("consultations-folders")
...
class TestFindThemesEndpoint:
    def test_find_themes_success(self, client, admin_user_token, free_text_question):
        ...
        with patch("consultation_analyser.data_pipeline.batch.submit_job") as mock_submit:
            ...
            assert response.status_code == 202
            assert "Find Themes job started" in response.json()["message"]
            assert response.json()["consultation_code"] == consultation.code
    ...
    def test_find_themes_no_free_text_questions(self, client, admin_user_token, consultation):
        """Test find themes endpoitns requires free text questions"""
        ...
        assert response.status_code == 400
        assert "no free-text questions" in response.json()["error"].lower()
...
class TestAssignThemesEndpoint:
    def test_assign_themes_success(self, client, admin_user_token, free_text_question):
        ...
        with (
            ...
            assert response.status_code == 202
            assert "Assign Themes job started" in response.json()["message"]

            # Verify themes were exported first
    ...
    def test_assign_themes_no_selected_themes(self, client, admin_user_token, free_text_question):
        ...
        with patch(
            ...
            assert response.status_code == 400
            assert "No selected themes found" in response.json()["error"]
