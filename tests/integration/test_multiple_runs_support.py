...
def test_viewing_multiple_processing_runs_as_staff(django_app):
    ...
    started_at_str = timezone.localtime(pr1.started_at).strftime("%-d %B %Y at %H:%M")
    finished_at_str = timezone.localtime(pr2.finished_at).strftime("%-d %B %Y at %H:%M")
    ...
    assert frontend_first_run_page.request.url.endswith(
        f"/consultations/{consultation.slug}/runs/{pr1.slug}/"
    )
